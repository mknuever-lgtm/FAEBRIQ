...
- **Print system (reverted 2026/09/25, supersedes the same-day Inter experiment):** Bodoni Moda, white, for every tagline design. This is the founder-approved system: line spacing is derived automatically from each line's own rendered font size (`GAP_L1_L2_OF_L1_SIZE` / `GAP_L2_BAR_OF_L2_SIZE` in `tools/make_print_file.py`), not a fixed ratio, so it self-adjusts per design and never needs per-design tuning. Reference: `assets/print-art/off-the-clock-still-iconic-light-4500.png`, the exact file the founder reviewed and approved on 2026/09/10 (CHANGELOG). Any future regeneration of a shared-lockup design must be visually checked against that file's spacing before it ships — this file is what "the spacing between the text lines and the bar" means. The bar is thin and near-continuous (hairline gaps between the 6 blocks), about 1.03x the widest text line, not the flat 1.35x no-gap bar from the 2026/09/25 Inter experiment.
