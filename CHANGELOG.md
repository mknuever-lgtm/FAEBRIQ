...
2026-09-25 Regenerated the 4 apparel masters PR #30 left untouched (code-it-serve-it, deploying-identity-v2, its-not-a-bug-its-me, off-the-clock-still-iconic) via tools/make_print_file.py --product apparel, bringing them onto the current LINE2_RATIO_DEFAULT=0.80 system; re-ran validate_sticker_final_system.py.
Why: apparel was still on the 2026-09-10 per-design pixel-shift pass (75%/85% split) while stickers moved to the uniform ratio-based system in PR #30; founder asked to bring apparel onto the same mechanism. All 4 now measure ~0.80-0.81, matching the sticker versions of the same designs.
Note: validator run also found sticker-final-system-2026-08-27/manifest.json stale against PR #30's own sticker regeneration (never re-validated after that pass); corrected as a byproduct. No Printify/Shopify sync performed.
