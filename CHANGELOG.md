...
2026-09-25 Added tools/composite_print.py and tools/make_model_composites.sh, and rebuilt the three on-model shots as assets/model-{tee,hoodie,cap}-print.png with the real print masters composited on: heal the generated print, perspective-warp the master in, displace along the fabric folds, shade by the cloth's own luminance.
Why: the generated prints were invented art, a smooth gradient instead of the six-block pride bar, and on the cap a swoosh across the brim the locked cap spec does not have. They read as stickers, not ink.
Note: the model-*-new.png originals are untouched and nothing is wired to the new files yet. Quads were set by eye per photo, so a replaced source photo needs its quad and roi re-tuned (--debug draws both).
