# FÆBRIQ: product images on the reverted print system (supersedes MUSE_BRIEF_4)

Paste this whole file into Muse instead of MUSE_BRIEF_4.md. Written 2026/09/25.

## Stop first, read this before anything else
Maurice reverted the print font. Every tagline design goes back to **Bodoni Moda serif** (the original system). The Inter SemiBold files in `system-2026-09-25/` are retired for taglines, stickers, the tote and the sheet. **The cap and sleeve logo are unaffected: they stay Inter SemiBold**, that was never in question.

If you already generated Tier 2 images against the old Inter files: **discard them, they're all invalid now**, regardless of whether they passed review. If you already touched Printify (uploaded, saved, or published any design), **stop and tell Maurice exactly what you saved, on which products, before doing anything else.** Don't publish anything using an Inter tagline file. Don't revert it yourself; wait for confirmation.

## Where things stand otherwise
Manus never delivered anything. **Printify is your job**, through your Printify connector. Code It Tee may have had its artwork deleted by Manus, so check it first. You have no Shopify connector, and Shopify isn't your job: Claude does it.
...
## Inputs
- Repo `mknuever-lgtm/FAEBRIQ`, branch `claude/hopeful-goldberg-a40dkh`. **Two folders now**: `assets/print-art/` (root, the serif tagline masters) and `assets/print-art/system-2026-09-25/` (only the two logo files below, still current). These are the only print files allowed. Never edit their text or proportions.
- Preview of every serif design: no single contact sheet yet; open each file listed below directly.

| Shopify handle | Product (all blanks black) | Print file (font: Bodoni Moda serif unless noted) |
|---|---|---|
| `404-straight-not-found-tee` | heavy cotton tee | `assets/print-art/404-straight-not-found-v3-light-4500.png` |
| `code-it-serve-it-tee` | heavy cotton tee | `assets/print-art/code-it-serve-it-light-4500.png` |
| `off-the-clock-still-iconic-hoodie` | heavyweight hoodie | `assets/print-art/off-the-clock-still-iconic-light-4500.png` (this is the spacing reference, see below) |
| `deploying-identity-v2-crewneck` | heavyweight crewneck | `assets/print-art/deploying-identity-v2-light-4500.png` |
| `404-straight-not-found-tote` | canvas tote | `assets/print-art/404-straight-not-found-v3-tote-4500.png` (same lockup as the 404 sticker, FÆBRIQ under the stripe) |
| `faebriq-baseball-cap-pride-rainbow-low-profile-hat` | low-profile unstructured cap, fabric strap | `assets/print-art/system-2026-09-25/logo-faebriq-1p35.png` (Inter SemiBold, unchanged) |
| `404-straight-not-found-sticker` | kiss-cut sticker | `assets/print-art/sticker-final-system-2026-08-27/404-straight-not-found-v3-sticker-light-2400.png` |
| `its-not-a-bug-its-me-sticker` | kiss-cut sticker | `assets/print-art/sticker-final-system-2026-08-27/its-not-a-bug-its-me-sticker-light-2400.png` |
| `code-it-serve-it-sticker` | kiss-cut sticker | `assets/print-art/sticker-final-system-2026-08-27/code-it-serve-it-sticker-light-2400.png` |
| `please-hold-sticker` | kiss-cut sticker | `assets/print-art/sticker-final-system-2026-08-27/please-hold-rebranding-identity-sticker-light-2400.png` |
| `deploying-identity-v2-sticker` | kiss-cut sticker | `assets/print-art/sticker-final-system-2026-08-27/deploying-identity-v2-sticker-light-2400.png` |
| `faebriq-sticker-sheet-full-drop` | sticker sheet | `assets/print-art/sticker-sheet-serif-2400x3600.png` |

**Spacing rule, applies to every serif design:** the gap between line 1 and line 2, and between line 2 and the bar, must read exactly like `off-the-clock-still-iconic-light-4500.png`. That file is the founder-approved reference; every other design was generated with the same formula, so they already match it. Don't add any manual spacing.

Tees, hoodie and crewneck also carry `assets/print-art/system-2026-09-25/wordmark-faebriq-sleeve.png` (Inter, FÆBRIQ alone, no stripe) about 3 in wide at the **bottom of the left sleeve**, **horizontal**: it reads left to right across the sleeve, level when the arm hangs down, about 1 in above the sleeve hem (tees) or the cuff (hoodie, crewneck). Show it where the angle allows.
...
3. **Upload by URL**, never from a device: `https://raw.githubusercontent.com/mknuever-lgtm/FAEBRIQ/claude/hopeful-goldberg-a40dkh/` + the file path from the table above (most are under `assets/print-art/`, only the cap and sleeve files are under `assets/print-art/system-2026-09-25/`).
...
- No print files other than the ones named in the table above.
