...
- **Bar-to-wordmark ratio (set 2026/09/23, reconfirmed 2026/09/25):** wherever the wordmark actually prints (stickers, cap, the standalone logo lockup), its bar is 1.35x the wordmark's own width, no gaps, `BAR_TO_MARK_RATIO` in `tools/make_print_file.py`. Apparel prints carry no wordmark, so their bar stays tied to the phrase width, nothing to be 1.35x of.
